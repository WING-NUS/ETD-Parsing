...
# 6. Cheap "shuffle" the indices
for d in $(ls folds/); do echo folds/$d && sort -R folds/$d/train_style_idx.txt > folds/$d/train_style_idx_shuffled.txt; done
for d in $(ls folds/); do echo folds/$d && sort -R folds/$d/val_style_idx.txt > folds/$d/val_style_idx_shuffled.txt; done
# 7. Generate seq2seq data from the indices
python generate_seq2seq_data_from_idx.py
# 8. Simple check on labels
cut -d$'\t' -f 2 folds/*/train.txt | sort | uniq
cut -d$'\t' -f 2 folds/*/val.txt | sort | uniq
# 9. Create compressed archive for training and validation data
